#'
#' This function lists all the conflicts between packages in psygold
...
psygold_conflicts <- function() {
  ...
  tidy_names <- paste0("package:", psygold_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% tidy_names))

  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)

  structure(conflict_funs, class = "psygold_conflicts")
}

psygold_conflict_message <- function(x) {
  if (length(x) == 0) return("")

  header <- cli::rule(
    left = crayon::bold("Conflicts"),
    right = "psygold_conflicts()"
  )
...
#' @export
print.psygold_conflicts <- function(x, ..., startup = FALSE) {
  cli::cat_line(psygold_conflict_message(x))
}
