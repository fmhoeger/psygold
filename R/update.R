#' Update psygold packages
#'
#' This will check to see if all psygold packages (and optionally, their
#' dependencies) are up-to-date, and will install after an interactive
#' confirmation.
#'
#' @inheritParams psygold_deps
#' @export
#' @examples
#' \dontrun{
#' psygold_update()
#' }
psygold_update <- function(recursive = FALSE, repos = getOption("repos")) {

  deps <- psygold_deps(recursive, repos)
  behind <- dplyr::filter(deps, behind)

  if (nrow(behind) == 0) {
    cli::cat_line("All psygold packages up-to-date")
    return(invisible())
  }
...
#' @export
psygold_deps <- function(recursive = FALSE, repos = getOption("repos")) {
  pkgs <- utils::available.packages(repos = repos)
  deps <- tools::package_dependencies("psygold", pkgs, recursive = recursive)
